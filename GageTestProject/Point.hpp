...
struct point
{
    ...
    void point::getCurrPos(std::unique_ptr<msr::airlib::MultirotorRpcLibClient>& client)
    {
    ...
    void point::getCurrPos(msr::airlib::MultirotorState state)
    {
    ...
    /// </summary>
    /// <returns></returns>
    msr::airlib::Vector3r point::toVector3r()
    {
    ...
    point point::makePoint(float nX, float nY, float nZ)
    {
        this->x = nX;
        this->y = nY;
        this->z = nZ;
    ...
    point point::copy(msr::airlib::Vector3r v)
    {
        this->x = v.x();
        this->y = v.y();
        this->z = v.z();
    ...
    /// <param name="otherP"></param>
    /// <returns></returns>
    const double point::distanceTo(point otherP)
    {
    ...
    /// <param name="otherP"></param>
    /// <returns></returns>
    const point point::vectorDistanceTo(point otherP)
    {
        point p;
        p.copy(*this);
        p.x = p.x - otherP.x;
    ...
    /// <param name="otherP"></param>
    /// <returns></returns>
    point point::vectorAbsDistanceTo(point otherP)
    {
        point p;
        p.copy(*this);
