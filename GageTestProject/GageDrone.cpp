...
GageDrone::GageDrone(std::string name, point start, point end)
{
    assert(start.z == 0);
...
GageDrone::GageDrone(std::string name, point start)
{
    assert(start.z == 0);
    this->name = name;
    this->start = start;
    start.z = STANDARD_HEIGHT;
    this->end = start;
    this->makeFinalPoint();
}

GageDrone:: GageDrone(std::string name, point start, float height)
{
    assert(start.z == 0);
    this->name = name;
    this->start = start;
